...
int main() {
    ...
    std::cout << "Zigbee End Device Example\n";

    zigbee_mesh::core::Config config;
    config.set<uint8_t>("device", "role", 2);
    ...
    network.setRole(zigbee_mesh::core::DeviceRole::EndDevice);
    network.start();

    if (network.joinNetwork(0x1234, 11)) {
        std::cout << "Joined network as End Device\n";
